-- Migrate Better Auth tables from auth schema to public and drop auth schema.
-- Run this once with: psql $DATABASE_URL -f prisma/migrations/migrate_auth_to_public.sql
-- Requires: auth schema and tables (auth.users, auth.accounts, auth.sessions, etc.) exist.

BEGIN;

-- 1) Ensure public.users exists (Better Auth + raw_user_meta_data for onboarding)
...
-- 2) Ensure public.accounts exists (Better Auth)
...
-- 3) Ensure public.sessions exists (Better Auth)
...
-- 4) Ensure public.verification exists (Better Auth OTP etc.)
...
-- 5) Ensure public.jwks exists (Better Auth JWT)
...
-- 6) Copy from auth to public only if auth schema exists and has data
DO $$
BEGIN
  ...
  THEN
    -- Expects auth.users with snake_case: id, name, email, email_verified, image, created_at, updated_at, raw_user_meta_data
...
-- 7) Drop auth schema (and all auth tables). Uncomment the next line when ready to remove auth.
